...
namespace TRSF.Invoicing.Demo.Web
{
    ...
    /// <summary>
    /// Busqueda por prefijo sobre el catalogo curado de la sesion, para el autocompletado
    /// del wizard. Deliberadamente separado de ICatalogValidator (Existe/ObtenerDescripcion
    /// es un contrato de validacion, no de navegacion) - la validacion real de lo que el
    /// usuario elige aqui sigue pasando por ICatalogValidator.Existe antes de sellar.
    /// </summary>
    public static class CatalogBrowser
    {
        ...
        public static List<CatalogMatch> BuscarPorPrefijo(string dbPath, CatalogoGrande catalogo, string prefijo, int limite = 20)
        {
            ...
            command.CommandText = $"SELECT Codigo, Descripcion FROM {Tables[catalogo]} WHERE Codigo LIKE @prefijo ORDER BY Codigo LIMIT @limite";
